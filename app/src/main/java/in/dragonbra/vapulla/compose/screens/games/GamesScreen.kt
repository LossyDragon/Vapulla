    ...
    GamesScreenContent(
        ...
        onItemClick = { game ->
            Intent(Intent.ACTION_VIEW).apply {
                val url = String.format(Constants.STORE_PAGE_URL, game.appid)
                data = Uri.parse(url)
...
private fun GamesScreenContent(
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.padding(paddingValues)) {
            if (state.filteredGameList.isEmpty()) {
...
private fun Preview_GamesScreenContent() {
    val state = GamesState(name = "Mr. Friendly", filteredGameList = listOf())
    VapullaTheme {
        GamesScreenContent(
            state,
            onItemClick = {},
